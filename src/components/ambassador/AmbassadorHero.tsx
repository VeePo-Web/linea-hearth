...
const AmbassadorHero = () => {
  ...
  return (
    <section className="relative min-h-screen bg-foreground text-background overflow-hidden flex flex-col justify-center hero-noise">
      {/* Background index number */}
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none">
        <span className="text-[50vw] md:text-[35vw] font-extralight tracking-[-0.02em] leading-none opacity-[0.03] text-background">
          01
        </span>
      </div>
      ...
      <div className="relative z-10 px-6 md:px-12 lg:px-24 py-32 md:py-40">
          ...
          <div className="flex flex-wrap gap-x-[0.25em]">
            {headlineWords.map((word, index) => (
              <motion.span
                ...
                className="text-hero-massive-mobile md:text-hero-massive text-background"
              >
                {word}
              </motion.span>
            ))}
        ...
        <motion.div
          ...
          className="max-w-2xl"
        >
          <p className="text-editorial text-background/70 mb-6">
          ...
          <p className="text-editorial text-background/70">
      ...
      >
        <span className="text-eyebrow text-background/40">Scroll to apply</span>
        <motion.div
          animate={{ y: [0, 8, 0] }}
          transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut" }}
        >
          <ChevronDown className="w-5 h-5 text-background/40" />
        </motion.div>
      </motion.div>
    </section>
  );
